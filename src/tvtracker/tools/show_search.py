...
def register(server: MCPServer) -> None:
    ...
    async def show_search(
        ...
        """Найти сериал по названию или по описанию сюжета.

        По описанию ищет там, где TVmaze не умеет: «сериал про офис, где сотрудникам
        разделяют память». Каждый результат помечен твоим состоянием отслеживания.
        """
        context = app(ctx)
        repo = context.repo
